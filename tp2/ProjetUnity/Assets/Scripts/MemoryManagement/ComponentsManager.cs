...
internal class ComponentsManager : Singleton<ComponentsManager>
{
    ...
    public void ForEach<T1>(Action<EntityComponent, T1> lambda) where T1 : IComponent
    {
        ...
        for (int i = 0; i < allEntitiesSize; i++)
        {
            if (t1Size > i)
            {
                ...
                t1Entity = t1List[i];
                if (entity != null && t1Entity != null)
                {
                    lambda((EntityComponent)entity, (T1)t1Entity);
                }
            }
            else
            {
    ...
    public void ForEach<T1, T2>(Action<EntityComponent, T1, T2> lambda) where T1 : IComponent where T2 : IComponent
    {
        ...
        IComponent t1Entity, t2Entity, entity;
        for (int i = 0; i < allEntitiesSize; i++)
        {
            if (t1Size > i && t2Size > i)
            {
                entity = allEntities[i];
                t1Entity = t1List[i];
                t2Entity = t2List[i];
                if (t1Entity != null && t2Entity != null && entity != null)
                {
                    lambda((EntityComponent)entity, (T1)t1Entity, (T2)t2Entity);
                }
            }
            else
            {
    ...
    public void ForEach<T1, T2, T3>(Action<EntityComponent, T1, T2, T3> lambda) where T1 : IComponent where T2 : IComponent where T3 : IComponent
    {
        ...
        IComponent t1Entity, t2Entity, t3Entity, entity;
        for (int i = 0; i < allEntitiesSize; i++)
        {
            if (t1Size > i && t2Size > i && t3Size > i)
            {
                entity = allEntities[i];
                t1Entity = t1List[i];
                t2Entity = t2List[i];
                t3Entity = t3List[i];
                if (entity != null && t1Entity != null && t2Entity != null && t3Entity != null)
                {
                    lambda((EntityComponent)entity, (T1)t1Entity, (T2)t2Entity, (T3)t3Entity);
                }
            }
            else
            {
    ...
    public void ForEach<T1, T2, T3, T4>(Action<EntityComponent, T1, T2, T3, T4> lambda) where T1 : IComponent where T2 : IComponent where T3 : IComponent where T4 : IComponent
    {
        ...
        IComponent t1Entity, t2Entity, t3Entity, t4Entity, entity;
        for (int i = 0; i < allEntitiesSize; i++)
        {
            if (t1Size > i && t2Size > i && t3Size > i && t4Size > i)
            {
                entity = allEntities[i];
                t1Entity = t1List[i];
                t2Entity = t2List[i];
                t3Entity = t3List[i];
                t4Entity = t4List[i];
                if (entity != null && t1Entity != null && t2Entity != null && t3Entity != null && t4Entity != null)
                {
                    lambda((EntityComponent)entity, (T1)t1Entity, (T2)t2Entity, (T3)t3Entity, (T4)t4Entity);
                }
            } 
            else
            {
